const { ReceivedLocals } = require("@/src/model");
const { badRequest, serverError, notFound } = require("@/src/utils/error");

//*: CREATE NEW
const createNew = async (body) => {
  const user = await ReceivedLocals.create(body)
    .then((user) => {
      return user.toJSON();
    })
    .catch((err) => {
...
const findItemById = async (id) => {
  const user = await ReceivedLocals.findOne({ where: { id: id }, raw: true });
...
const updateItemById = async (id, bodyData) => {
  ...
  const updatedData = await ReceivedLocals.findOne({ where: { id: id } })
    .then((data) => {
      if (!data) {
        throw notFound("Item not found");
      }
...
const deleteItemById = async (id, bodyData) => {
  const deletedItem = await ReceivedLocals.findOne({ where: { id: id } })
    .then(async (item) => {
      if (!item) {
        throw notFound("Item not found");
      }
...
const findAllItems = async () => {
  const users = await ReceivedLocals.findAll({ raw: true });
